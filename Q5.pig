...
moviesyear = FILTER movies BY year  >2014 and year<2017;
jnd = join moviesyear by movieid, moviegenres by movieid;
grp = group jnd by (genre,year);
DESCRIBE grp;
cnt = foreach grp generate ($0),COUNT($1);
srtd = order cnt by $0 asc;
final = foreach srtd generate flatten($0),$1;
